...
- `MIN_DETECT_ZOOM = 16` -- the floor below which nothing here does anything at all, not even
  ...
  multiplier to 4x -- matches `app/server/config.json`'s `min_detect_zoom`, which was
...
## vite.config.ts (notes moved out of code comments, 2026-09-23)

The dev server proxies `/api` and `/ws` to the backend on 8010 (`ws: true` is required -- Vite's
proxy doesn't upgrade websockets on its own); production uses nginx for the same job
