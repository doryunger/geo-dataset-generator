"""
Run the live site classifier offline over whole sites and report the verdict per site: which
component types matched, whether oil_refinery was identified, and what the verdict would be
without a given component. Uses the server's own detection batch and classifier code, so what it
reports is what the map would show.

    python oil_refinery/eval_sites.py --class distillation-column --without distillation-column
"""
import argparse
import asyncio
import json
...
sys.path.insert(0, str(REPO_ROOT / "oil_refinery" / "app" / "server"))
