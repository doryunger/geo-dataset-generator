...
def qualifying_keys(
    ...
    """(tile_id, index) of every detection that counts toward a satisfied requirement.

    A detection can be drawn and still not count: a fan-unit with no other fan within the class's
    group_within_m, or a class whose members never reach min_count.
    """
    edges = {e["to"]: e for e in graph["edges"] if e["relation"] == "requires"}
