...
detections on the Brunsb�ttel check batch (6 fan-units, 20 vs 19 storage tanks, 2 chimneys). Four
...
(Brunsb�ttel tile 1 went from 6 fans to none). Batches are spatially local (viewport, centre-out)
...
`tile_server.prune_pending()` and stops awaiting. Verified on the Brunsb�ttel batch: identified
