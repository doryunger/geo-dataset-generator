...
Status: the graph itself is implemented -- `app/server/semantic_graph.json` (data) and
...
- **`geometry.py`** (`app/server/`) already computes global-pixel-space distance
...
**Implemented** (`app/server/semantic_graph.json` + `site_graph.py`). Superseded two
...
and `scripts/context/scripts.md`). `chimney` stays the one exception in the
